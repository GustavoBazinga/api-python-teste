...
historio_de_compras: Dict[int, List[int]] = {}
...
def criar_produto(produto: CriarProduto) -> Produto:
    ...
    Args:
        produto (CriarProduto): Objeto do tipo CriarProduto
...
def adicionar_historico_compras(
    ...
    historio_de_compras[usuario_id] = compras.produtos_ids
...
def recomendar_produtos(usuario_id: int, preferencias: Preferencias) -> List[Produto]:
    ...
    """
    if usuario_id not in historio_de_compras:
        raise HTTPException(
    ...
    produtos_recomendados = [
        produto
        for produto_id in historio_de_compras[usuario_id]
